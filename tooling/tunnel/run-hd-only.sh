#!/bin/bash
# Run HD tunnel tests only. Assumes HD is already running and mesh is up.
...
log "===== HD tunnel tests @ ${VCPU} vCPU ====="
for n in $TUNNELS; do
  for r in 1 2 3 4 5; do
    log "T1: hd ${VCPU}v ${n}t r${r}"
    run "$n" "$r" "${RESULTS}/T1_scaling/${VCPU}vcpu/hd/${n}t_r${r}"
  done
done
log "===== HD ${VCPU} vCPU done ====="
