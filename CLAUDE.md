...
Two workstations with 50GbE NICs (not SmartNIC) arriving in 1-2 weeks. These will expose
completely different bottlenecks: at 50Gbps with 1400B packets, need to sustain ~4.5M pps,
which is ~220ns per-packet budget. NIC-level optimizations (RSS, interrupt coalescing,
real kTLS hardware paths) become relevant. The VM results are the conservative case.
