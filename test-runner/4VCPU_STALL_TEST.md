# 4 vCPU Backpressure Stall — Test Protocol

Run this after the fix has been applied to Hyper-DERP and the new binary deployed to the relay VM. The fix changes backpressure thresholds in `types.h` and the recv_pause check in `data_plane.cc`. See `~/dev/HD-bench/4VCPU_STALL_FIX.md` for the full analysis.
