...
Sustained-load characterization of `mode: wireguard` with the XDP fast path attached. Measured against the libvirt fleet (`hd-r2` relay, `hd-c1`/`hd-c2` clients), single virtio bridge, native-mode XDP on `virtio_net`. Receiver tuned per the [quickstart](https://github.com/hyper-derp/Hyper-DERP/blob/master/docs/wireguard_relay_quickstart.md) (`net.core.rmem_max=32M`, iperf3 `-w 8M`); without that the receiver overflows and you measure iperf3 instead of the relay.
